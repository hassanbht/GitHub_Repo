import 'package:github_repo_app/core/infrastructure/sembast_database.dart';
import 'package:github_repo_app/github/core/infrastructure/github_headers.dart';
...
class GithubHeadersCache {
  ...
  Future<GithubHeaders?> getHeaders(Uri uri) async {
    final json =
        await _store.record(uri.toFilePath()).get(_sembastDatabase.instance);
    return json == null ? null : GithubHeaders.fromJson(json);
  }

  Future<void> deleteHeaders(Uri uri) async {
    await _store.record(uri.toFilePath()).delete(_sembastDatabase.instance);
  }
}
